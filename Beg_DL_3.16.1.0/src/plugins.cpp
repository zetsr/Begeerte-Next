...
#define NUMERIC_COMPARE(op_char) \
    ...
    namespace Plugins {
        ...
        void RegisterEntityListAPI(ScriptContext& context) {
            ...
            /*
            context.RegisterFunction("Player_GetGender", [](std::vector<Value>& args) -> Value {
                if (args.size() != 1 || args[0].GetType() != Value::Type::PLAYER_PTR) {
                    throw std::runtime_error("Player_GetGender requires 1 Player object argument.");
                }
                EntityList::Player* p = args[0].AsPlayer();
                if (!p) return Value("Error: Null Player");
                const wchar_t* w_gender = p->GetGender();
                char buffer[256];
                size_t convertedChars = 0;
                wcstombs_s(&convertedChars, buffer, sizeof(buffer), w_gender, _TRUNCATE);
        ...
        void Init() {
            ...
            std::thread script_thread([tasks = std::move(tasks)]() mutable {
                ...
                for (auto& task : tasks) {
                    ...
                    ScriptContext context(task.path); // Create a new context for each script

                    // Register common functions
                    context.RegisterFunction("printf", Printf);
                    context.RegisterFunction("print", Print);
                    context.RegisterFunction("LogToFile", LogToFile); // Renamed for clarity

                    // Register EntityList API for this script
                    RegisterEntityListAPI(context);

                    if (!g_cheatdata) {
                        std::string error = "[BegeerteScript] FATAL: g_cheatdata not initialized before script execution. Aborting script: " + task.path;
                    ...
                    try {
                        ...
                        std::string error = "[BegeerteScript] Unhandled exception during execution of " +
                            std::filesystem::path(task.path).filename().string() + ": " + e.what();
                        std::cerr << error << std::endl;
                        task.error = error;
                        // Log error to file
                        {
                        ...
                        // Script is not marked as executed, effectively "unloaded"
                    }
                    std::cout << "---" << std::endl;
                }
                ...
                for (const auto& task : tasks) {
                    if (!task.executed && !task.error.empty()) {
                        std::cout << "[BegeerteScript] Script failed and was unloaded: " << task.path << std::endl;
                    }
                }
                });
            ...
            std::cout << "[BegeerteScript] Initialization complete. Scripts are running in a separate thread." << std::endl;
        }
